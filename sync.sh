...
PARAMS=$(aws ssm get-parameters --names "/myRewardsApp/$APP_ENV/customerUserPoolId" "/myRewardsApp/$APP_ENV/customerWebClientId" "/myRewardsApp/$APP_ENV/identityPoolIdCustomer" "/myRewardsApp/$APP_ENV/appConfig" "/myRewardsApp/$APP_ENV/googleMapsKey" --with-decryption --region us-east-1 --profile "$selected_profile" 2>/dev/null)
...
if [[ $? -ne 0 ]]; then
    ...
    PARAMS=$(aws ssm get-parameters --names "/myRewardsApp/$APP_ENV/customerUserPoolId" "/myRewardsApp/$APP_ENV/customerWebClientId" "/myRewardsApp/$APP_ENV/identityPoolIdCustomer" "/myRewardsApp/$APP_ENV/appConfig" "/myRewardsApp/$APP_ENV/googleMapsKey" --with-decryption --region us-east-1 --profile "$selected_profile")
...
GOOGLE_KEY=$(echo "$PARAMS" | jq -r ".Parameters[] | select(.Name==\"/myRewardsApp/$APP_ENV/googleMapsKey\").Value")
